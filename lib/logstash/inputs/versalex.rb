...
class LogStash::Inputs::VersaLex < LogStash::Inputs::Base
  ...
  def register
    ...
    @logfile ||= Cleo::VersaLex::default_log(@service)
    if @logfile.nil?
      if @service.nil?
  ...
  def run(queue)
    ...
    eventlog = Cleo::VersaLex::EventLog.new @logfile, :follow=>true, :bufsize=>@buffer_size, :sleep=>@interval, :logger=>@logger
