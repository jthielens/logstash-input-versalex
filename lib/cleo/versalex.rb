...
module Cleo::VersaLex
  #----------------------------------------------------------------------------#
  # VersaLex XML Event Log parser/tailer.  Use it like this:                   #
  #   eventlog = Cleo::VersaLex::EventLog.new "file", :follow=>true            #
...
if __FILE__ == $0
  ...
  class Laster
    ...
    def update(event)
      ...
      elsif event.is_a? Cleo::VersaLex::Event
        @events.push event
  ...
  class Dumper
    ...
    def update(event)
      if event.is_a? Cleo::VersaLex::Event
        if @skip > 0
          @skip -= 1
        elsif @skip == 0
          @callback.call event
  ...
  begin
    parser.parse!
    options[:file] = ARGV.shift || Cleo::VersaLex::default_log(options[:service])
    if ARGV.length>0
  ...
  #----------------------------------------------------------------------------#
  eventlog = Cleo::VersaLex::EventLog.new options[:file], :follow=>options[:follow]
